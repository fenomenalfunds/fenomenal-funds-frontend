...
import {destroyUser, redirectUser} from "./auth";
...
export async function securePostAPI(token, endpoint, variables) {
	const parseJSON = resp => (resp.json ? resp.json() : resp);
 ...
 * @param {object} variables
 */
export async function fetchSecureGraphQL(ctx, token, query, variables = {}) {
	const requestUrl = getStrapiURL();
	const response = await fetch(`${requestUrl}/graphql`, {
		method: 'POST',
		headers: {
	...
	if(json.errors){
		console.error('Applicaton errors: ', json.errors);
		if(json.errors[0].message === 'Invalid token.') {
...
export async function fetchResourcesContent(ctx, token, start = 0, preview = {}) {
	const result = await fetchSecureGraphQL(ctx, token, `
	query fetchResourcesContent{
	  ...
	  resources: resourceArticles(start:${start}, limit: 8,sort:"publish:DESC"){
	    title
	    slug
	    subtitle
	    thumbnail{
 ...
 * @param {slug} slug content slug
 * @param {object} preview preview content while loading
 * @returns {Promise<{related: *, document: *}>}
 */
export async function fetchResourceDetail(ctx, token, slug, preview = {}) {
	const result = await fetchSecureGraphQL(ctx, token, `
	query fetchResourceDetail {
	  ...
	  related: resourceArticles(where:{slug_nin:"${slug}"}){
	    title
	    subtitle
	    slug
	    thumbnail{
...
export async function fetchGrantArticleDetail(slug, tags = [], preview = {}) {
	...
	query fetchGrantArticleDetail{
	  article:grantArticles(where:{slug:"${slug}"}){
	    title
	    subtitle
	    body
	    image{
	  ...
	  related: grantArticles(where:{slug_nin:"${slug}"}){
	    title
	    slug
	    subtitle
	    thumbnail{
...
export async function fetchProfileContent(token, id) {
	return await fetchSecureAPI(token, `/users/me`);
}
